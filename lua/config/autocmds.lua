  ...
  callback = function()
    ...
    if ok_maven and maven.find_pom() then
      wk.add({
        ...
        { "<leader>jmD", function() maven.run_maven_cmd("mvn spring-boot:run -Dspring-boot.run.jvmArguments=\"-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=*:5005\"") end, desc = "Spring Boot Debug (Port 5005)", icon = " " },
    ...
    if ok_gradle and gradle.find_gradle() then
      wk.add({
        ...
        { "<leader>jgD", function() gradle.run_gradle_cmd("./gradlew bootRun --debug-jvm") end, desc = "Spring Boot Debug (Port 5005)", icon = " " },
