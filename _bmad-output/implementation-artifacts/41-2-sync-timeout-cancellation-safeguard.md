...
- Reverted `SYNC_TIMEOUT_MS` back to `120000` in `maven.lua` after testing (it was never changed in `gradle.lua`, which was implemented symmetrically but not independently timeout-tested against a real process — see Completion Notes).
...
- Discovered and documented (Debug Log above) a real subtlety in `vim.system` + pipe-capturing behavior: killing the tracked process is not sufficient if a grandchild process is forked (not exec'd) and inherits the stdout/stderr pipes — the exit callback can hang indefinitely even after the tracked PID is confirmed dead. This does **not** affect real `mvnw`/`gradlew` wrapper scripts, which `exec` into `java` rather than forking, so no additional process-group-kill handling was added — flagging this as a known edge case rather than in-scope hardening, since addressing it generically (process groups/sessions) is meaningfully more complexity than this story's stated scope.
- `gradle.lua`'s change is symmetric to `maven.lua`'s and passed `nvim --headless "+Lazy check" +qa`, but the timeout/kill path itself was only *directly* process-level-verified against `maven.lua`'s `sync_dependencies()` (both files share identical logic shape, so this is a reasonable confidence level, not a full duplicate test run).
...
- `lua/cumulus/util/maven.lua` (modified — `SYNC_TIMEOUT_MS`, timer, `timed_out` flag, timeout notification branch)
