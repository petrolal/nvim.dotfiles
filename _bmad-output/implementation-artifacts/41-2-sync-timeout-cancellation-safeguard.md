...
- 2026-08-08: Post-review fix — found and closed a narrow race: `timed_out = true` and `handle:kill()` were set/called inside `vim.schedule_wrap(...)`, deferring them to the next main-loop tick instead of the instant the libuv timer fires. If the process's own (unscheduled) exit callback happened to run in that gap, it would see `timed_out == false`, report success/failure normally, and moments later the timeout branch would *also* fire — two contradictory notifications back to back. Fix: set the flag and call `handle:kill()` synchronously in the raw timer callback (both are libuv-level operations, safe outside the main loop); only `vim.notify`/`mark_ready()` still go through `vim.schedule`. Re-verified the timeout, fast-success, and fast-failure paths all still behave correctly after the change.
